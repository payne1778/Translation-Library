...
public class Translator {
    ...
    public static void setPythonScriptPath(String newPythonScriptPath) {
        if (isEmptyParam(newPythonScriptPath, "setPythonScriptPath")) return;
    ...
    public static void setPreferredLanguage(String newPreferredLanguage) {
        if (isEmptyParam(newPreferredLanguage, "setPreferredLanguage")) return;
        if (
            !getAvailableLanguagesList().contains(newPreferredLanguage) ||
            !getAnglicizedAvailableLanguagesList()
                .contains(newPreferredLanguage)
        ) {
            System.out.printf(
     ...
     */
    private static boolean isEmptyParam(
        String paramToCheck,
        String methodName
    ) {
        if (paramToCheck.isEmpty()) {
            System.out.println(
                "The parameter passed to " + methodName + " was empty."
            );
            printCurrentFields();
            return true;
        }
    ...
    public static List<String> getAvailableLanguagesList() {
        ...
        ArrayList<String> command = new ArrayList<>(
            Arrays.asList(pythonInterpreter, pythonScriptPath, "list")
        );
    ...
    private static String runCommand(ArrayList<String> command) {
        ...
        try {
            ...
            if (exitCode != 0) {
                ...
                errorReader.close();
                outputLines += ("An error occurred when interacting with " +
                    pythonScriptPath +
                    ". " +
